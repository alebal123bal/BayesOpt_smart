...
class ProgressLogger:
    ...
    def __init__(self, log_file: Optional[str] = None, verbose: bool = True):
        ...
        self.prev_n_evaluations = 0  # Track previous evaluation count for batch detection
    ...
    def __call__(self, state):
        ...
        if self.verbose:
            ...
            for i in range(batch_start, n_evaluations):
                x_str = np.array2string(x_values[i], precision=1, suppress_small=True, separator=',')
                y_str = np.array2string(y_values[i], precision=4, suppress_small=True, separator=',')
                print(f"      Point {i+1}: x={x_str} → objectives={y_str}")
            
            # Show global bests
            best_strs = []
            for obj_idx in range(n_objectives):
                x_str = np.array2string(self.best_x_per_objective[obj_idx], precision=1, suppress_small=True, separator=',')
                best_strs.append(f"obj{obj_idx}={self.best_per_objective[obj_idx]:.4f} at {x_str}")
        ...
        if self.log_file:
            with open(self.log_file, 'a', encoding='utf-8') as f:
                f.write(f"{state['iteration']},{state['n_evaluations']},"
...
class PerformanceMonitor:
    ...
    def summary(self):
        ...
        print("\n  Breakdown (average):")
        print(f"    Hyperparams: {np.mean(self.hyperparams_times):.4f}s "
              f"({100*np.mean(self.hyperparams_times)/np.mean(self.timings):.1f}%)")
        print(f"    Kernels: {np.mean(self.kernels_times):.4f}s "
              f"({100*np.mean(self.kernels_times)/np.mean(self.timings):.1f}%)")
        print(f"    Acquisition: {np.mean(self.acquisition_times):.4f}s "
              f"({100*np.mean(self.acquisition_times)/np.mean(self.timings):.1f}%)")
        print(f"    Evaluation: {np.mean(self.eval_times):.4f}s "
...
class ConvergenceChecker:
    ...
    def __init__(self, tolerance: float = 1e-4, patience: int = 3, verbose: bool = True):
        """
